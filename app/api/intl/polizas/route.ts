...
WITH base AS (
  SELECT
    NUMBER, STATUS_QUOTE, PAYMENT_STATUS, ANNUAL_PREMIUM,
    INSURED_VALUE, MASTER_AGENCY, LEVEL_2, ISSUE_DATE, DATE_CHANGE, UPDATED
...
latest AS (
  SELECT *, ROW_NUMBER() OVER (PARTITION BY NUMBER ORDER BY DATE_CHANGE DESC, UPDATED DESC) AS rn
  FROM base
)
SELECT
...
FROM latest
WHERE rn = 1 AND ISSUE_DATE IS NOT NULL
GROUP BY 1, 2, 3, 4
ORDER BY 1 DESC
`;
...
export async function GET() {
  ...
  try {
    ...
    const data = await runQuery<IntlPolizaRow>(SQL, accessToken);
